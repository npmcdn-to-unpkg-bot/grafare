...
func updateProcessHelper(processId int64,form dtos.UpdateProcessForm) Response {
  logger := log.New("main")
  logger.Info("updatedProcess1 %s")
  cmd := m.UpdateProcessCommand{ProcessId:processId,ProcessName: form.ProcessName, ParentProcessName: form.ParentProcessName,UpdatedBy:form.UpdatedBy}
...
func GetProcessByProcessId(c *middleware.Context) Response {
  logger := log.New("main")
  logger.Info("GetProcess123 %s")
  processId:=c.ParamsInt64(":processId")

  return getProcessUser(processId)
}
func getProcessUser(processId int64) Response {
  query := m.GetProcessByCodeQuery{ProcessId:processId}
